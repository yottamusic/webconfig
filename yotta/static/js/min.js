...
fetch('/cgi-bin/hifi/version').then(function (resp) {
  return resp.text();
}).then(function (text) {
...
if (yottaCarIP) {
fetch('http://'+yottaCarIP+'/cgi-bin/hifi/version').then(function (resp) {
  return resp.text();
}).then(function (text) {
	if (text.startsWith('yottaCar')) {
...
function recommend() {
    newSongPage = 2;
    var htmlString = "";
    if (recomData.data.length > 0 && recomData.data != null) {
        for (var j = 0, l = recomData.data.length; j < l; j++) {
            var src = recomData.data[j]["imgurl"];
            var text = recomData.data[j]["cname"];
            var description = recomData.data[j]["description"];
            htmlString += "<li cid='" + recomData.data[j]["cid"] + "' _type='" + recomData.data[j]["ctype"] + "' title='" + recomData.data[j]["cname"] + "' onclick='showNewSongsList(this)'>";
            htmlString += "<div cid='" + recomData.data[j]["cid"] + "' _type='" + recomData.data[j]["ctype"] + "'  title='" + recomData.data[j]["cname"] + "' class='more gobal_bg' onclick='showNewSongsList(this)'><i></i></div>";
            htmlString += "<div class='pic'><img src='" + defaultImg + "' _src='" + src + "' width='38' height='38' alt=''/></div>";
            htmlString += "<div class='textfm'>" + text.intercept(30, "...") + "<span>" + description.intercept(40, "") + "</span></div>";
            htmlString += "</li>"
        }
        htmlString += "<li class='m' onclick='getNewSongListData()'>更多</li>";
        var el = Kg.$$("#content > ul")[0];
        el.className = "";
        el.innerHTML = htmlString;
        window.scrollTo(0, 1)
    } else {
        htmlString += "<li class='m'>抱歉，没有新歌或者获取数据出错</li>"
    }
    Kg.$("#recommend").innerHTML = htmlString
}

function getNewSongListData() {
    var myScript = document.createElement("script");
    myScript.src = "http://mobilecdn.kugou.com/new/app/i/quku.php?cmd=106&page=" + newSongPage + "&pagesize=8&outputtype=jsonp&callback=dealNewSongListData&d=" + (new Date).getTime();
    var header = document.getElementsByTagName('body')[0];
    header.appendChild(myScript)
}

function dealNewSongListData(o) {
    ...
    if (o.data.length > 0 && o.data != null) {
        for (var j = 0, l = o.data.length; j < l; j++) {
            var src = o.data[j]["imgurl"];
            var text = o.data[j]["cname"];
            var description = o.data[j]["description"];
            htmlString += "<li cid='" + o.data[j]["cid"] + "' _type='" + o.data[j]["ctype"] + "' title='" + o.data[j]["cname"] + "' onclick='showNewSongsList(this)'>";
            htmlString += "<div cid='" + o.data[j]["cid"] + "' _type='" + o.data[j]["ctype"] + "'  title='" + o.data[j]["cname"] + "' class='more gobal_bg' onclick='showNewSongsList(this)'><i></i></div>";
            htmlString += "<div class='pic'><img src='" + defaultImg + "' _src='" + src + "' width='38' height='38' alt=''/></div>";
            htmlString += "<div class='textfm'>" + text.intercept(30, "...") + "<span>" + description.intercept(40, "") + "</span></div>";
            htmlString += "</li>"
        }
...
function showNewSongsList(dom) {
    getNewSongListSongData(dom.getAttribute("_type"), dom.getAttribute("cid"), 1, 20, "dealNewSongData");
    Kg.$(".textTitle p").innerHTML = dom.getAttribute("title");
    Kg.$(".textTitle p").setAttribute("cid", dom.getAttribute("cid"));
    Kg.$(".textTitle p").setAttribute("type", dom.getAttribute("_type"))
}

function getNewSongListSongData(type, cid, page, pagesize, callback) {
    var myScript = document.createElement("script");
    myScript.src = "http://mobileservice.kugou.com/new/app/i/quku.php?cmd=107&ctype=" + type + "&cid=" + cid + "&page=" + page + "&pagesize=" + pagesize + "&outputtype=jsonp&callback=" + callback + "&d=" + (new Date).getTime();
    var header = document.getElementsByTagName('body')[0];
    header.appendChild(myScript)
}

function dealNewSongData(o) {
    page = 1;
    focusSong = o.data;
    ...
    if (o.data.length > 0 && o.data != null) {
        ...
        if (o.data.length > 20) {
            lishtml += "<li class='m' onclick='getNewSongDataMore()'>更多</li>"
        }
        el.className = "";
...
function getNewSongDataMore() {
    getNewSongListSongData(Kg.$(".textTitle p").getAttribute("type"), Kg.$(".textTitle p").getAttribute("cid"), page, 20, "dealNewSongDataMore")
}

function dealNewSongDataMore(o) {
    var wait = document.createElement("li");
    wait.className = "m loading";
    var el = Kg.$$("#content > ul")[10];
    var more = el.getElementsByClassName("m")[0];
    ...
    if (o.data.length > 0 && o.data != null) {
        ...
        lishtml += "<li class='m'>已经是最后一页了</li>"
    }
    el.removeChild(wait);
    el.className = "";
    el.innerHTML += lishtml
...
function getMoreInfo() {
    ...
    if (currentTab == 0) {
        ...
        src = yottaCar+"/cgi-bin/hifi/proxy?http://m.kugou.com/app/i/fmList.php";
        params = "pagesize=" + pageSize + "&pageindex=" + page
    }
    setTimeout(function() {
        Kg.get(src, params, function(o) {
...
function getFmList(initList) {
    ...
    var str = "";
    setTimeout(function() {
        Kg.get(yottaCar+"/cgi-bin/hifi/proxy?http://m.kugou.com/app/i/fmList.php", {
            "pageindex": 1,
            "pagesize": pageSize
        }, function(o) {
            o = new Function("return " + o)();
...
function showLayer(e, o) {
    ...
    if (o == showLayer.currentObj && el.offsetHeight > 0) {
        ...
        down.onclick = function() {
            ...
            Kg.$('#down_client_layer') && (Kg.$('#down_client_layer').style.display = 'block')
        };
        play = down = null
    }
...
function shutLoginbox() {
    try {
        ...
        Kg.$("#reset_layer").style.display = "none"
    } catch (e) {}
}
...
document.onclick = function() {
    ...
    Kg.$('#layer3') && (Kg.$('#layer3').style.display = 'none');
    Kg.$('#layer4') && (Kg.$('#layer4').style.display = 'none');
    Kg.$('#reset_layer') && (Kg.$("#reset_layer").style.display = "none");
    Kg.$('#down_client_layer') && (Kg.$('#down_client_layer').style.display = 'none')
};
var kugouid;
var kugouName;
var userpic;
...
function dealMyList(o) {
    ...
    listData = o;
    var html = "<div class='userInfo'><img src='" + getUserIcon(userpic) + "' width='52' height='52'/><p><label id='username'>" + kugouName + "</label><a class='admin' onclick='showLayer4(event,this)'>管理</a></p><p><span id='syntips'>正在同步中...</span><a class='logout' href='http://www1.kugou.com/user/logout.aspx?url=http://m.kugou.com' onclick='logout()'>退出</a></p></div>";
    var htmlselect = "";
    var songNums = 0;
...
function login() {
    if (document.getElementById("userName").value == "") {
        ...
        Kg.getJSON("../app/i/loginAndReg.php", {
            ...
            "captcha": document.getElementById("userCode").value,
            "cmd": 400
        }, function(result) {
            if (result.status == 1) {
                alert("登录成功！");
...
function chageCode(o) {
    o.src = "http://m.kugou.com/app/i/captcha.php?" + Math.random()
}
...
function showResetPsd() {
    Kg.stopEvent(event);
    Kg.$("#reset_layer").style.display = "block"
}
try {
...
function diskList() {
    ...
    var el = Kg.$$("#content > ul")[1];
    el.style.display = "block";
    getData(100, "/media", page, topListSize, "returnLists")
}
...
function sdnClick1(num) {
    setTimeout(function() {
        (new Image()).src = "/front/link.php?id=" + num + "&url=&t=" + Math.random()
    }, 0)
};
LRC = function() {
    this.initialize.apply(this, arguments)
...
if (!!(document.createElement('audio').canPlayType("audio/mpeg"))) {
    ...
    document.getElementById("playButton").onclick = function() {
        if (myplayer.getSongsNum() == 1 && myplayer.songs[0].hash == 0) {
            document.getElementById("poptips").innerHTML = "请进入列表再播放歌曲";
